...
| Eps -> (fun _ -> Some [])
| Zero -> (fun _ -> None)
| Char c -> (fun c' -> if char_dec c (Obj.magic c') then Some (c :: []) else None)
| Any -> (fun c -> Some ((Obj.magic c) :: []))
| Cat (g1, g2) ->
  (fun p ->
    coq_Bind (Obj.magic coq_OptionMonad) (pretty_print g1 (fst (Obj.magic p))) (fun s1 ->
      coq_Bind (Obj.magic coq_OptionMonad) (pretty_print g2 (snd (Obj.magic p))) (fun s2 -> coq_Return (Obj.magic coq_OptionMonad) (app s1 s2))))
| Alt (g1, g2) ->
  (fun v ->
  ...
  let rec loop v =
    ...
    | hd :: tl ->
      coq_Bind (Obj.magic coq_OptionMonad) (pretty_print g0 hd) (fun s' ->
        match s' with
        | [] -> None
        | _ :: _ -> coq_Bind (Obj.magic coq_OptionMonad) (Obj.magic loop tl) (fun s -> coq_Return (Obj.magic coq_OptionMonad) (app s' s)))
  in loop
| Map (fi, g0) -> (fun v -> coq_Bind (Obj.magic coq_OptionMonad) (snd (Obj.magic fi) v) (fun x -> pretty_print g0 x))
...
(** val map : bigrammar -> ((interp -> interp) * (interp -> interp option)) -> bigrammar **)
...
| BLeaf (_, g, _) -> g
| BNode (_, gt1, gt2) -> let g1 = comb_bigrammar gt1 in let g2 = comb_bigrammar gt2 in alt g1 g2
...
let perm2 p1 p2 =
  map (alt (seq p1 p2) (seq p2 p1)) ((fun v ->
    ...
    | Coq_inr y -> let (b, a) = y in Obj.magic (a, b)), (fun u -> let (a, b) = Obj.magic u in Some (Obj.magic (Coq_inl (a, b)))))
...
let option_perm2 p1 p2 =
  map (alt empty (alt (seq p1 (option_perm p2)) (seq p2 (option_perm p1)))) ((fun v ->
    match Obj.magic v with
    | Coq_inl _ -> Obj.magic (None, None)
    | Coq_inr y ->
      (match y with
       | Coq_inl y0 -> let (a, ob) = y0 in Obj.magic ((Some a), ob)
       | Coq_inr y0 -> let (b, oa) = y0 in Obj.magic (oa, (Some b)))), (fun u ->
    let (y, y0) = Obj.magic u in
    (match y with
     | Some a -> Some (Obj.magic (Coq_inr (Coq_inl (a, (snd (Obj.magic u))))))
     | None ->
       (match y0 with
...
let option_perm3 p1 p2 p3 =
  map (alt (alt empty (seq p1 (option_perm2 p2 p3))) (alt (seq p2 (option_perm2 p1 p3)) (seq p3 (option_perm2 p1 p2)))) ((fun v ->
    match Obj.magic v with
    | Coq_inl y ->
      (match y with
       | Coq_inl _ -> Obj.magic (None, (None, None))
       | Coq_inr y0 -> let (a, y1) = y0 in let (ob, oc) = y1 in Obj.magic ((Some a), (ob, oc)))
    | Coq_inr y ->
      (match y with
       | Coq_inl y0 -> let (b, y1) = y0 in let (oa, oc) = y1 in Obj.magic (oa, ((Some b), oc))
       | Coq_inr y0 -> let (c, y1) = y0 in let (oa, ob) = y1 in Obj.magic (oa, (ob, (Some c))))), (fun u ->
    let (oa, u1) = Obj.magic u in
    let (ob, oc) = u1 in
    (match oa with
     | Some a -> Some (Obj.magic (Coq_inl (Coq_inr (a, (snd (Obj.magic u))))))
     | None ->
       (match ob with
...
let option_perm3_variation p1 p2 p3 =
  map (alt (seq p1 (option_perm2_variation p2 p3)) (alt (seq p2 (option_perm2_variation p1 p3)) (seq p3 (option_perm2 p1 p2)))) ((fun v ->
    match Obj.magic v with
    | Coq_inl y -> let (a, y0) = y in let (ob, c) = y0 in Obj.magic ((Some a), (ob, c))
    | Coq_inr y ->
      (match y with
       | Coq_inl y0 -> let (b, y1) = y0 in let (oa, c) = y1 in Obj.magic (oa, ((Some b), c))
       | Coq_inr y0 -> let (c, y1) = y0 in let (oa, ob) = y1 in Obj.magic (oa, (ob, c)))), (fun u ->
    let (oa, u1) = Obj.magic u in
    let (ob, c) = u1 in
    (match oa with
...
| Any -> Grammar.Any
| Cat (g1, g2) -> Grammar.Cat ((bigrammar_to_grammar g1), (bigrammar_to_grammar g2))
| Alt (g1, g2) -> Grammar.Alt ((bigrammar_to_grammar g1), (bigrammar_to_grammar g2))
