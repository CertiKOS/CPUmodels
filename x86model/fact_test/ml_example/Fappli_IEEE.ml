...
let loc_of_shr_record mrs =
  ...
  if shr_r0 then if shr_s0 then Coq_loc_Inexact Gt else Coq_loc_Inexact Eq else if shr_s0 then Coq_loc_Inexact Lt else Coq_loc_Exact
...
(** val shr_fexp : Big.big_int -> Big.big_int -> Big.big_int -> Big.big_int -> location -> shr_record * Big.big_int **)

let shr_fexp prec emax =
  let emin = Z.sub (Z.sub (Big.doubleplusone Big.one) emax) prec in
  let fexp = coq_FLT_exp emin prec in (fun m e l -> shr (shr_record_of_loc m l) e (Z.sub (fexp (Z.add (coq_Zdigits2 m) e)) e))
...
(** val binary_round_aux : Big.big_int -> Big.big_int -> mode -> bool -> Big.big_int -> Big.big_int -> location -> full_float **)

let binary_round_aux prec emax mode0 sx mx ex lx =
  let (mrs', e') = shr_fexp prec emax mx ex lx in
  let (mrs'', e'') = shr_fexp prec emax (choice_mode mode0 sx mrs'.shr_m (loc_of_shr_record mrs')) e' Coq_loc_Exact in
  (Big.z_case
     (fun _ -> F754_zero
     sx)
     (fun m ->
...
(** val coq_Bmult :
    Big.big_int -> Big.big_int -> (binary_float -> binary_float -> bool * nan_pl) -> mode -> binary_float -> binary_float -> binary_float **)
   ...
   | B754_finite (sx, mx, ex) ->
     (match y with
      ...
      | B754_finite (sy, my, ey) ->
        (match binary_round_aux prec emax m (xorb sx sy) (Pos.mul mx my) (Z.add ex ey) Coq_loc_Exact with
         | F754_zero s -> B754_zero s
         | F754_infinity s -> B754_infinity s
         | F754_nan (b, pl) -> B754_nan (b, pl)
         | F754_finite (s, m0, e) -> B754_finite (s, m0, e))))

(** val shl_align : Big.big_int -> Big.big_int -> Big.big_int -> Big.big_int * Big.big_int **)
...
(** val shl_align_fexp : Big.big_int -> Big.big_int -> Big.big_int -> Big.big_int -> Big.big_int * Big.big_int **)

let shl_align_fexp prec emax =
  let emin = Z.sub (Z.sub (Big.doubleplusone Big.one) emax) prec in
  let fexp = coq_FLT_exp emin prec in (fun mx ex -> shl_align mx ex (fexp (Z.add (digits2_pos mx) ex)))

(** val binary_round : Big.big_int -> Big.big_int -> mode -> bool -> Big.big_int -> Big.big_int -> full_float **)

let binary_round prec emax m sx mx ex =
  let (mz, ez) = shl_align_fexp prec emax mx ex in binary_round_aux prec emax m sx mz ez Coq_loc_Exact

(** val binary_normalize : Big.big_int -> Big.big_int -> mode -> Big.big_int -> Big.big_int -> bool -> binary_float **)
   ...
   | B754_finite (sx, mx, ex) ->
     (match y with
      ...
      | B754_finite (sy, my, ey) ->
        ...
        binary_normalize prec emax m (Z.add (cond_Zopp sx (fst (shl_align mx ex ez))) (cond_Zopp sy (fst (shl_align my ey ez)))) ez
          (match m with
           | Coq_mode_DN -> true
           | _ -> false)))
...
(** val coq_Fdiv_core_binary : Big.big_int -> Big.big_int -> Big.big_int -> Big.big_int -> Big.big_int -> (Big.big_int * Big.big_int) * location **)
...
let coq_Fdiv_core_binary prec m1 e1 m2 e2 =
  ...
  (Big.z_case
     (fun _ ->
     let (q, r) = coq_Zfast_div_eucl m1 m2 in ((q, e), (new_location m2 r Coq_loc_Exact)))
     (fun p ->
     let m = Z.shiftl m1 p in let e' = Z.add e (Big.opp p) in let (q, r) = coq_Zfast_div_eucl m m2 in ((q, e'), (new_location m2 r Coq_loc_Exact)))
     (fun _ ->
     let (q, r) = coq_Zfast_div_eucl m1 m2 in ((q, e), (new_location m2 r Coq_loc_Exact)))
     (Z.sub (Z.add d2 prec) d1))

(** val coq_Bdiv : Big.big_int -> Big.big_int -> (binary_float -> binary_float -> bool * nan_pl) -> mode -> binary_float -> binary_float -> binary_float **)
