...
| Zero -> re_and_fn Coq_rZero (Obj.magic (fun _ -> assert false (* absurd case *)))
| Char c -> re_and_fn (Coq_rChar c) (fun x -> x)
| Any -> re_and_fn Coq_rAny (fun x -> x)
| Cat (g1, g2) ->
  let ag1,f1 = split_grammar g1 in
  let ag2,f2 = split_grammar g2 in re_and_fn (Coq_rCat (ag1, ag2)) (fun p -> Obj.magic ((f1 (fst (Obj.magic p))), (f2 (snd (Obj.magic p)))))
| Alt (g1, g2) ->
  let ag1,f1 = split_grammar g1 in
...
| Star g0 -> let ag,f = split_grammar g0 in re_and_fn (Coq_rStar ag) (fun xs -> Obj.magic map f xs)
| Map (f1, g0) -> let ag,f2 = split_grammar g0 in re_and_fn ag (fun x -> f1 (f2 x))
