...
module MakeRaw =
 ...
 struct
  ...
  | Leaf -> Big.zero
  | Node (_, l, _, r) -> Big.succ (add (cardinal l) (cardinal r))

  (** val maxdepth : tree -> Big.big_int **)

  let rec maxdepth = function
  | Leaf -> Big.zero
  | Node (_, l, _, r) -> Big.succ (max (maxdepth l) (maxdepth r))

  (** val mindepth : tree -> Big.big_int **)

  let rec mindepth = function
  | Leaf -> Big.zero
  | Node (_, l, _, r) -> Big.succ (min (mindepth l) (mindepth r))

  (** val for_all : (elt -> bool) -> tree -> bool **)

  let rec for_all f = function
  | Leaf -> true
  | Node (_, l, x, r) -> if if f x then for_all f l else false then for_all f r else false

  (** val exists_ : (elt -> bool) -> tree -> bool **)

  let rec exists_ f = function
  | Leaf -> false
  | Node (_, l, x, r) -> if if f x then true else exists_ f l then true else exists_ f r
  ...
  | Node (lh, ll, lx, lr) ->
    (fun x ->
      ...
      | Leaf -> add x l
      | Node (rh, rl, rx, rr) ->
        if I.ltb (I.add rh I._2) lh then bal ll lx (join lr x r) else if I.ltb (I.add lh I._2) rh then bal (join_aux rl) rx rr else create l x r
      in join_aux)
  ...
  let rec remove_min l x r =
    match l with
    | Leaf -> (r, x)
    | Node (_, ll, lx, lr) -> let (l', m) = remove_min ll lx lr in ((bal l' x r), m)
  ...
  let merge s1 s2 =
    ...
    | Node (_, _, _, _) ->
      (match s2 with
       | Leaf -> s1
       | Node (_, l2, x2, r2) -> let (s2', m) = remove_min l2 x2 r2 in bal s1 m s2')
  ...
  let concat s1 s2 =
    ...
    | Node (_, _, _, _) ->
      (match s2 with
       | Leaf -> s1
       | Node (_, l2, x2, r2) -> let (s2', m) = remove_min l2 x2 r2 in join s1 m s2')
  ...
  | Node (_, l, y, r) ->
    (match X.compare x y with
     ...
     | Lt -> let { t_left = ll; t_in = b; t_right = rl } = split x l in { t_left = ll; t_in = b; t_right = (join rl y r) }
     | Gt -> let { t_left = rl; t_in = b; t_right = rr } = split x r in { t_left = (join l y rl); t_in = b; t_right = rr })
    ...
    | Node (_, l1, x1, r1) ->
      (match s2 with
       ...
       | Node (_, _, _, _) ->
         ...
         if pres then join (inter l1 l2') x1 (inter r1 r2') else concat (inter l1 l2') (inter r1 r2'))
  ...
  let rec diff s1 s2 =
    ...
    | Node (_, l1, x1, r1) ->
      (match s2 with
       ...
       | Node (_, _, _, _) ->
         let { t_left = l2'; t_in = pres; t_right = r2' } = split x1 s2 in
         if pres then concat (diff l1 l2') (diff r1 r2') else join (diff l1 l2') x1 (diff r1 r2'))
  ...
  let rec union s1 s2 =
    ...
    | Node (_, l1, x1, r1) ->
      (match s2 with
       | Leaf -> s1
       | Node (_, _, _, _) -> let { t_left = l2'; t_in = _; t_right = r2' } = split x1 s2 in join (union l1 l2') x1 (union r1 r2'))

  (** val filter : (elt -> bool) -> tree -> tree **)

  let rec filter f = function
  | Leaf -> Leaf
  | Node (_, l, x, r) -> let l' = filter f l in let r' = filter f r in if f x then join l' x r' else concat l' r'
  ...
  | Leaf -> (Leaf, Leaf)
  | Node (_, l, x, r) ->
    let (l1, l2) = partition f l in let (r1, r2) = partition f r in if f x then ((join l1 x r1), (concat l2 r2)) else ((concat l1 r1), (join l2 x r2))
