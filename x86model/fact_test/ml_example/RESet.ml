...
| Raw.Leaf -> Coq_rZero
| Raw.Node (_, lft, r, rgt) -> Coq_rAlt ((tree_to_regexp lft), (Coq_rAlt (r, (tree_to_regexp rgt))))
...
| Raw.Node (_, l, x, r) ->
  ...
  app (map (fun x0 -> Obj.magic (Coq_inl x0)) vl)
    (app (map (fun x0 -> Obj.magic (Coq_inr (Coq_inl x0))) vx) (map (fun x0 -> Obj.magic (Coq_inr (Coq_inr x0))) vr))
...
(** val create_xform : Raw.tree -> xform -> regexp -> xform -> Raw.tree -> xform -> tree_xf_pair **)

let create_xform l fl x fx r fr =
  (Raw.Node ((Int.Z_as_Int.add (Int.Z_as_Int.max (Raw.height l) (Raw.height r)) Big.one), l, x, r)),(xmatch fl (xmatch fx fr))
...
let rec inject_xform_tree t1 t2 =
  match t1 with
  | Raw.Leaf -> Xzero
  | Raw.Node (_, t_left0, r, t_right0) -> xmatch (inject_xform_tree t_left0 t2) (xmatch (find_xform t2 r) (inject_xform_tree t_right0 t2))
...
(** val assert_false_xform : Raw.tree -> xform -> regexp -> xform -> Raw.tree -> xform -> tree_xf_pair **)

let assert_false_xform l fl x fx r fr =
  create_xform l fl x fx r fr

(** val bal_xform : Raw.tree -> xform -> regexp -> xform -> Raw.tree -> xform -> tree_xf_pair **)
...
let bal_xform l fl x fx r fr =
  ...
  then (match l with
        ...
        | Raw.Node (_, ll, lx, lr) ->
          ...
          then let r',fr' = create_xform lr (xcomp (xcomp Xinr Xinr) fl) x fx r fr in
               create_xform ll (xcomp Xinl fl) lx (xcomp (xcomp Xinl Xinr) fl) r' fr'
          else (match lr with
                | Raw.Leaf -> assert_false_xform l fl x fx r fr
                | Raw.Node (_, lrl, lrx, lrr) ->
                  let l',fl' = create_xform ll (xcomp Xinl fl) lx (xcomp (xcomp Xinl Xinr) fl) lrl (xcomp (xcomp Xinl (xcomp Xinr Xinr)) fl) in
                  let r',fr' = create_xform lrr (xcomp (xcomp Xinr (xcomp Xinr (xcomp Xinr Xinr))) fl) x fx r fr in
                  create_xform l' fl' lrx (xcomp (xcomp Xinl (xcomp Xinr (xcomp Xinr Xinr))) fl) r' fr'))
       ...
       then (match r with
             ...
             | Raw.Node (_, rl, rx, rr) ->
               ...
               then let l',f' = create_xform l fl x fx rl (xcomp Xinl fr) in
                    create_xform l' f' rx (xcomp (xcomp Xinl Xinr) fr) rr (xcomp (xcomp Xinr Xinr) fr)
               else (match rl with
                     | Raw.Leaf -> assert_false_xform l fl x fx r fr
                     | Raw.Node (_, rll, rlx, rlr) ->
                       let l',fl' = create_xform l fl x fx rll (xcomp (xcomp Xinl Xinl) fr) in
                       let r',fr' =
                         create_xform rlr (xcomp (xcomp Xinr (xcomp Xinr Xinl)) fr) rx (xcomp (xcomp Xinl Xinr) fr) rr (xcomp (xcomp Xinr Xinr) fr)
                       in
                       create_xform l' fl' rlx (xcomp (xcomp Xinl (xcomp Xinr Xinl)) fr) r' fr'))
...
let rec add_xform_tree x f1 s f2 =
  match s with
  | Raw.Leaf -> (Raw.Node (Big.one, Raw.Leaf, x, Raw.Leaf)),(xcomp (xmatch Xzero (xmatch Xid Xzero)) f1)
  | Raw.Node (h, l, y, r) ->
                 ...
                 (xcomp
                   (match xcomp (xcomp Xinl Xinr) f2 with
                    ...
                    | x0 ->
                      (match f1 with
                       | Xzero -> Xzero
                       | x1 -> Xpair (x0, x1))) (xopt (xfoldr (Xcons (Xfst, (xcoerce Xsnd))) Xsnd Xfst))) (xcomp (xcomp Xinr Xinr) f2)))
     | Lt -> let l',f' = add_xform_tree x f1 l (xcomp Xinl f2) in bal_xform l' f' y (xcomp (xcomp Xinl Xinr) f2) r (xcomp (xcomp Xinr Xinr) f2)
     | Gt -> let r',f' = add_xform_tree x f1 r (xcomp (xcomp Xinr Xinr) f2) in bal_xform l (xcomp Xinl f2) y (xcomp (xcomp Xinl Xinr) f2) r' f')

(** val join_xform : Raw.tree -> xform -> regexp -> xform -> Raw.tree -> xform -> Raw.tree*xform **)
    ...
    (fun x0 fx ->
      let rec join_aux r x1 =
        ...
        | Raw.Node (rh, rl, rx, rr) ->
          ...
          then let r',fr' = join_xform lr (xcomp (xcomp Xinr Xinr) x) x0 fx (Raw.Node (rh, rl, rx, rr)) x1 in
               bal_xform ll (xcomp Xinl x) lx (xcomp (xcomp Xinl Xinr) x) r' fr'
          else if Int.Z_as_Int.ltb (Int.Z_as_Int.add lh Int.Z_as_Int._2) rh
               then let l',fl' = join_aux rl (xcomp Xinl x1) in bal_xform l' fl' rx (xcomp (xcomp Xinl Xinr) x1) rr (xcomp (xcomp Xinr Xinr) x1)
               else create_xform (Raw.Node (lh, ll, lx, lr)) x x0 fx (Raw.Node (rh, rl, rx, rr)) x1
      in join_aux)

type triple_xform = { t_left : Raw.tree; t_left_xform : xform; t_in : xform option; t_right : Raw.tree; t_right_xform : xform }
...
let rec split_xform x s f =
  match s with
  | Raw.Leaf -> { t_left = Raw.Leaf; t_left_xform = Xzero; t_in = None; t_right = Raw.Leaf; t_right_xform = Xzero }
  | Raw.Node (_, l, y, r) ->
    (match REOrderedTypeAlt.compare x y with
     | Eq ->
       { t_left = l; t_left_xform = (xcomp Xinl f); t_in = (Some (xcomp (xcomp Xinl Xinr) f)); t_right = r; t_right_xform = (xcomp (xcomp Xinr Xinr) f) }
     | Lt ->
       let { t_left = ll; t_left_xform = fll; t_in = opt; t_right = rl; t_right_xform = frl } = split_xform x l (xcomp Xinl f) in
       let r',fr' = join_xform rl frl y (xcomp (xcomp Xinl Xinr) f) r (xcomp (xcomp Xinr Xinr) f) in
       { t_left = ll; t_left_xform = fll; t_in = opt; t_right = r'; t_right_xform = fr' }
     | Gt ->
       let { t_left = rl; t_left_xform = frl; t_in = opt; t_right = rr; t_right_xform = frr } = split_xform x r (xcomp (xcomp Xinr Xinr) f) in
       let l',fl' = join_xform l (xcomp Xinl f) y (xcomp (xcomp Xinl Xinr) f) rl frl in
       { t_left = l'; t_left_xform = fl'; t_in = opt; t_right = rr; t_right_xform = frr })

(** val union_xform_tree : Raw.tree -> xform -> Raw.tree -> xform -> Raw.tree*xform **)
...
let rec union_xform_tree s1 x s2 f2 =
  ...
  | Raw.Node (i, l1, x1, r1) ->
    (match s2 with
     ...
     | Raw.Node (_, _, _, _) ->
       let { t_left = l2'; t_left_xform = fl2'; t_in = opt; t_right = r2'; t_right_xform = fr2' } = split_xform x1 s2 f2 in
       let l',fl' = union_xform_tree l1 (xcomp Xinl x) l2' fl2' in
       let r',fr' = union_xform_tree r1 (xcomp (xcomp Xinr Xinr) x) r2' fr2' in
           ...
           xcomp
             (match xf1 with
              ...
              | x0 ->
                (match fother with
                 | Xzero -> Xzero
                 | x2 -> Xpair (x0, x2))) (xopt (xfoldr (Xcons (Xfst, (xcoerce Xsnd))) Xsnd Xfst))
         | None -> xf1
       in
...
let add_xform refx rs =
  let x,fx = refx in let s,fs = rs in let p = add_xform_tree x fx s fs in let t',ft' = p in t',ft'

(** val union_xform : rs_xf_pair -> rs_xf_pair -> rs_xf_pair **)

let union_xform rs1 rs2 =
  let s1,f1 = rs1 in let s2,f2 = rs2 in let p = union_xform_tree s1 f1 s2 f2 in let t0,f0 = p in t0,f0

(** val fold_tree_xform : (re_xf_pair -> 'a1 -> 'a1) -> Raw.tree -> xform -> 'a1 -> 'a1 **)

let rec fold_tree_xform f s x v =
  match s with
  | Raw.Leaf -> v
  | Raw.Node (_, l, x0, r) -> fold_tree_xform f r (xcomp (xcomp Xinr Xinr) x) (f (x0,(xcomp (xcomp Xinl Xinr) x)) (fold_tree_xform f l (xcomp Xinl x) v))
...
| _ -> RawRESet.fold (fun r1 s' -> RawRESet.add (Coq_rCat (r1, r)) s') s RawRESet.empty
...
| Coq_rEps -> let raw_set,f = s in raw_set,(xcomp f (xmap (Xpair (Xid, Xunit))))
