...
module Word =
 struct
  ...
  let signed wordsize_minus_one n =
    if zlt n (half_modulus wordsize_minus_one) then n else Z.sub n (modulus wordsize_minus_one)
  ...
  let lt wordsize_minus_one x y =
    if zlt (signed wordsize_minus_one x) (signed wordsize_minus_one y) then true else false
  ...
  let coq_Zdiv_round x y =
    if zlt x Big.zero
    then if zlt y Big.zero then Z.div (Z.opp x) (Z.opp y) else Z.opp (Z.div (Z.opp x) y)
  ...
  let divs wordsize_minus_one x y =
    repr wordsize_minus_one (coq_Zdiv_round (signed wordsize_minus_one x) (signed wordsize_minus_one y))

  (** val mods : Big.big_int -> int -> int -> int **)

  let mods wordsize_minus_one x y =
    repr wordsize_minus_one (coq_Zmod_round (signed wordsize_minus_one x) (signed wordsize_minus_one y))
  ...
  let coq_Z_shift_add b x =
    if b then Z.add (Z.mul (Big.double Big.one) x) Big.one else Z.mul (Big.double Big.one) x
  ...
  let rec bits_of_Z n x =
    Big.nat_case
      ...
      false)
      (fun m ->
      let (b, y) = coq_Z_bin_decomp x in let f = bits_of_Z m y in (fun i -> if zeq i Big.zero then b else f (Z.sub i Big.one)))
      n
  ...
  let rec coq_Z_of_bits n f =
    Big.nat_case
      ...
      Big.zero)
      (fun m ->
      coq_Z_shift_add (f Big.zero) (coq_Z_of_bits m (fun i -> f (Z.add i Big.one))))
      n

  (** val sig_cat : Big.big_int -> (Big.big_int -> bool) -> (Big.big_int -> bool) -> Big.big_int -> bool **)
  ...
  (** val bitwise_binop : Big.big_int -> (bool -> bool -> bool) -> int -> int -> int **)

  let bitwise_binop wordsize_minus_one f x y =
    let fx = bits_of_Z (wordsize wordsize_minus_one) x in
    let fy = bits_of_Z (wordsize wordsize_minus_one) y in repr wordsize_minus_one (coq_Z_of_bits (wordsize wordsize_minus_one) (fun i -> f (fx i) (fy i)))
  ...
  let shl wordsize_minus_one x y =
    let fx = bits_of_Z (wordsize wordsize_minus_one) x in repr wordsize_minus_one (coq_Z_of_bits (wordsize wordsize_minus_one) (fun i -> fx (Z.sub i y)))

  (** val shru : Big.big_int -> int -> int -> int **)

  let shru wordsize_minus_one x y =
    let fx = bits_of_Z (wordsize wordsize_minus_one) x in repr wordsize_minus_one (coq_Z_of_bits (wordsize wordsize_minus_one) (fun i -> fx (Z.add i y)))
  ...
  let rol wordsize_minus_one x y =
    let fx = bits_of_Z (wordsize wordsize_minus_one) x in
    repr wordsize_minus_one (coq_Z_of_bits (wordsize wordsize_minus_one) (fun i -> fx (Z.modulo (Z.sub i y) (Z.of_nat (wordsize wordsize_minus_one)))))

  (** val ror : Big.big_int -> int -> int -> int **)

  let ror wordsize_minus_one x y =
    let fx = bits_of_Z (wordsize wordsize_minus_one) x in
    repr wordsize_minus_one (coq_Z_of_bits (wordsize wordsize_minus_one) (fun i -> fx (Z.modulo (Z.add i y) (Z.of_nat (wordsize wordsize_minus_one)))))
 end
