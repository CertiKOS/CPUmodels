...
let new_location_odd nb_steps k l =
  ...
  then (match l with
        ...
        | Coq_loc_Inexact _ -> Coq_loc_Inexact Lt)
         ...
         (match Z.compare (Z.add (Z.mul (Big.double Big.one) k) Big.one) nb_steps with
          | Eq ->
            (match l with
             | Coq_loc_Exact -> Lt
...
let new_location nb_steps =
  if coq_Zeven nb_steps then new_location_even nb_steps else new_location_odd nb_steps
