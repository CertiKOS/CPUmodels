...
(** val coq_Zpos_div_eucl_aux1 : Big.big_int -> Big.big_int -> Big.big_int * Big.big_int **)
...
let rec coq_Zpos_div_eucl_aux1 a b =
  Big.positive_case
    ...
    Big.positive_case
      (fun a' ->
      let (q, r) = coq_Zpos_div_eucl_aux1 a' b' in (q, (Z.add (Z.mul (Big.double Big.one) r) Big.one)))
      (fun a' ->
      let (q, r) = coq_Zpos_div_eucl_aux1 a' b' in (q, (Z.mul (Big.double Big.one) r)))
      (fun _ -> (Big.zero,
      a))
      a)
    (fun _ -> (a,
...
(** val coq_Zpos_div_eucl_aux : Big.big_int -> Big.big_int -> Big.big_int * Big.big_int **)
...
(** val coq_Zfast_div_eucl : Big.big_int -> Big.big_int -> Big.big_int * Big.big_int **)
