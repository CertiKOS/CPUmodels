                   ...
                   (match xcomp f1 (xmap Xinl) with
                    ...
                    | x1 ->
                      (match xcomp f2 (xmap Xinr) with
                       | Xzero -> Xzero
                       | x2 -> Xpair (x1, x2))) (xopt (xfoldr (Xcons (Xfst, (xcoerce Xsnd))) Xsnd Xfst)))
       else true,(xcomp f1 (xmap Xinl))
  else let x0,f2 = nullable r2 in if x0 then true,(xcomp f2 (xmap Xinr)) else false,Xzero
...
| Coq_rChar b -> if char_dec a b then let rs,f = RES.singleton_xform Coq_rEps in rs,(xcomp f (xmap (xchar a))) else RES.empty_xform __
| Coq_rAny -> let rs,f = RES.singleton_xform Coq_rEps in rs,(xcomp f (xmap (xchar a)))
| Coq_rCat (r1, r2) ->
...
| Coq_rAlt (r1, r2) -> let rs1,f1 = pdrv a r1 in let rs2,f2 = pdrv a r2 in RES.union_xform (rs1,(xcomp f1 (xmap Xinl))) (rs2,(xcomp f2 (xmap Xinr)))
| Coq_rStar r1 -> let rsc,fc = RES.cat_re_xform (pdrv a r1) (Coq_rStar r1) in rsc,(xcomp fc (xmap (Xcons (Xfst, (xcoerce Xsnd)))))
| _ -> RES.empty_xform __

(** val pdrv_rex : char_p -> re_xf_pair -> rs_xf_pair **)

let pdrv_rex a = function
| r,f -> let rs,frs = pdrv a r in rs,(xcomp frs (xcomp (xmap f) (xopt (xfoldr (xopt (xfoldr (Xcons (Xfst, (xcoerce Xsnd))) Xsnd Xfst)) Xempty Xid))))

(** val pdrv_set : char_p -> rs_xf_pair -> rs_xf_pair **)

let pdrv_set a rx =
  RES.fold_xform (fun rex rx1 -> RES.union_xform (pdrv_rex a rex) rx1) rx (RES.empty_xform __)
...
let transition_to_vtransition _ t0 =
  { vrow_num = t0.row_num; vrow_entries = (Array.of_list t0.row_entries); vrow_nils = t0.row_nils }

(** val dfa_to_vdfa : coq_DFA -> vDFA **)

let dfa_to_vdfa d =
  { vdfa_num_states = d.dfa_num_states; vdfa_states = d.dfa_states; vdfa_transition =
    (Array.of_list (map (transition_to_vtransition d.dfa_states) d.dfa_transition)) }
  ...
  Big.nat_case
    ...
    (match get_index_wfs2 s1 ss with
     | Some s0 ->
       ...
       let entries,_ = r in
       let f_back = gen_backward_xform s1 ss' s0 in let e = { next_state = s0; next_xform = (xopt (xcomp f_back (xcoerce f1))) } in ss',((e :: entries),__)
     | None ->
       let ss',r = gen_row' s n' (add_wfs s1 ss) (Nat0.add (Big.succ Big.zero) tid) in
       let entries,_ = r in let e = { next_state = (cardinal_wfs ss); next_xform = (xcoerce f1) } in ss',((e :: entries),__)))
    n
...
let coerce_transitions _ _ ts =
  map (fun t0 -> { row_num = t0.row_num; row_entries = (map coerce_entry t0.row_entries); row_nils = (map coerce_nils t0.row_nils) }) ts

(** val cons_transition : Big.big_int -> wf_states -> entries_t -> transition_t **)

let cons_transition n ss entries =
  { row_num = n; row_entries = entries; row_nils = (RES.re_set_extract_nil (get_state n ss)) }
...
let rec build_table_func x =
  ...
  let build_table0 = fun ss0 rows0 next_state1 -> let y = ss0,(rows0,next_state1) in build_table_func y in
  ...
  (match filtered_var with
   | Some s0 ->
     ...
     let entries,_ = s in
     build_table0 ss' (app (coerce_transitions ss ss' rows) ((cons_transition next_state0 ss' entries) :: [])) (Nat0.add (Big.succ Big.zero) next_state0)
   | None -> ss,rows)
...
let build_dfa r =
  let ss,table = build_transition_table r in { dfa_num_states = (cardinal_wfs ss); dfa_states = ss; dfa_transition = table }
...
let naive_parse_token nps tk =
  ...
  let v = RES.re_set_extract_nil rs in ({ rx_nps = (rs,f); fixup_nps = nps.fixup_nps }, (flat_map (compose (Obj.magic map nps.fixup_nps) (xinterp f)) v))

(** val initial_naive_parser_state : grammar -> naiveParserState **)

let initial_naive_parser_state g =
  let r,f = split_grammar g in { rx_nps = (RES.singleton_xform r); fixup_nps = (coerce_dom f) }
...
let initial_parser_state' db g =
  ...
  { dfa_ps = (coerce_dfa (db r)); row_ps = Big.zero; fixup_ps = (coerce_dom (compose (Obj.magic map f) (xinterp f1))) }
  ...
  | Some s ->
    (match nth_error2 s.row_entries tk with
     | Some s0 ->
       ...
       let g = compose (Obj.magic flat_map next_fixup) (xinterp s0.next_xform) in
       let vs0 =
         match nth_error2 ps.dfa_ps.dfa_transition next_i with
         | Some s1 -> coerce s1.row_nils
         | None -> []
       in
       let vs' = flat_map g vs0 in ({ dfa_ps = ps.dfa_ps; row_ps = next_i; fixup_ps = g }, vs')
...
let ps_extract_nil ps =
  flat_map ps.fixup_ps (RES.re_set_extract_nil (get_state ps.row_ps ps.dfa_ps.dfa_states))
...
let vinitial_parser_state' db g =
  let r,f = split_grammar g in
  let f1 = projT2 (RES.singleton_xform r) in { vdfa_ps = (db r); vrow_ps = Big.zero; vfixup_ps = (coerce_dom (compose (Obj.magic map f) (xinterp f1))) }
...
let vparse_token ps tk =
  ...
  let row' = get ps.vdfa_ps.vdfa_transition next_i in let vs0 = coerce row'.vrow_nils in { vdfa_ps = ps.vdfa_ps; vrow_ps = next_i; vfixup_ps = g },vs0

(** val vps_extract_nil : vinstParserState -> __ list **)

let vps_extract_nil ps =
  flat_map' ps.vfixup_ps (RES.re_set_extract_nil (get_state ps.vrow_ps ps.vdfa_ps.vdfa_states))
