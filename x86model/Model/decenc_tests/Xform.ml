...
| Xcomp (f1, f2) -> let f1' = xinterp f1 in let f2' = xinterp f2 in (fun x0 -> f2' (f1' x0))
| Xchar c -> (fun _ -> Obj.magic c)
| Xunit -> (fun _ -> Obj.magic ())
| Xempty -> (fun _ -> Obj.magic [])
| Xpair (f1, f2) -> let f1' = xinterp f1 in let f2' = xinterp f2 in (fun x0 -> Obj.magic ((f1' x0), (f2' x0)))
| Xfst -> Obj.magic fst
| Xsnd -> Obj.magic snd
...
| Xmatch (f1, f2) ->
  ...
  | Coq_inl x1 -> f1' x1
  | Coq_inr x2 -> f2' x2)
| Xcons (f1, f2) -> let f1' = xinterp f1 in let f2' = xinterp f2 in (fun x0 -> Obj.magic ((f1' x0) :: (Obj.magic f2' x0)))
| Xfoldr (f1, f2, f3) ->
  let f1' = xinterp f1 in
  let f2' = xinterp f2 in let f3' = xinterp f3 in (fun x0 -> fold_right (fun a b -> Obj.magic f1' (a, b)) (f2' x0) (Obj.magic f3' x0))
...
let rec xfoldr' x3 x x0 =
  ...
  | Xzero -> Xzero
  | Xempty -> x0
  | Xmatch (x31, x32) -> Xmatch ((xfoldr' x31 x (Xcomp (Xinl, x0))), (xfoldr' x32 x (Xcomp (Xinr, x0))))
  | Xcons (x31, x32) ->
    Xcomp
         ...
         | x ->
           (match xcomp
            ...
            | x0 ->
              (match xcomp Xsnd Xsnd with
               | Xzero -> Xzero
               | x1 -> Xpair (x, (Xcons (x0, (xcoerce x1))))))) (xpair_fst Xempty) Xsnd) Xsnd)

(** val xcross : __ -> xform **)

let xcross _ =
  xopt
    (xcomp (xcomp (Xpair (Xsnd, Xfst)) (xmapenv (xcomp (Xpair (Xsnd, Xfst)) (xmapenv Xid))))
      (xopt (xfoldr (xopt (xfoldr (Xcons (Xfst, (xcoerce Xsnd))) Xsnd Xfst)) Xempty Xid)))
